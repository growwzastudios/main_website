...
import { Sparkles, TrendingUp, PhoneCall, Rocket, Lightbulb } from 'lucide-react';
...
const LandingPage = () => {
  ...
  const colors = {
    ...
    gold: {
      ...
      400: '#c9b28b', 500: '#b9a282', 600: '#a18d71',
    },
    stone: {
      200: '#e7e5e4', 300: '#d6d3d1', 400: '#a8a29e',
      500: '#78716c', 600: '#57534e', 700: '#44403c', 800: '#292520',
    }
  };

  const fadeInUp = {
    initial: { opacity: 0, y: motionSafe ? 60 : 20 },
    animate: { opacity: 1, y: 0 },
    transition: { duration: motionSafe ? 0.6 : 0.3, ease: 'easeOut' }
  };

  const fadeInRight = {
    initial: { opacity: 0, x: motionSafe ? 60 : 20 },
    animate: { opacity: 1, x: 0 },
    transition: { duration: motionSafe ? 0.8 : 0.3, ease: 'easeOut' }
  };

  const staggerContainer = {
    animate: { transition: { staggerChildren: motionSafe ? 0.2 : 0.08 } }
  };

  // Pulse only on desktop — constant scale animation is expensive on mobile
  const pulseAnimate = motionSafe
  ...
  return (
    ...
    >
      {/* Background blur orb — static on mobile, animated on desktop */}
      ...
      <motion.div
        style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 40, display: 'block' }}
        className="mobile-button"
        initial={{ opacity: 0, scale: 0 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.4, delay: 0.8 }}
      >
        <style>{`@media (min-width: 768px) { .mobile-button { display: none !important; } }`}</style>
        <Link to="/contact" style={{ textDecoration: 'none' }}>
          <motion.button
            style={{
              width: '56px', height: '56px', backgroundColor: colors.gold[400],
              color: colors.stone[800], borderRadius: '9999px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: 'none', cursor: 'pointer', boxShadow: '0 10px 20px rgba(0,0,0,0.15)',
            }}
            whileTap={{ scale: 0.9 }}
            animate={pulseAnimate}
          >
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px', width: '100%', zIndex: 10 }}>
          <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '60px', alignItems: 'center' }}>
            ...
            <motion.section initial="initial" animate="animate" variants={staggerContainer}>
              ...
              >
                <Link to="/contact" className="button-link">
                  <motion.button
                    ...
                    style={{
                      ...
                      backgroundColor: colors.stone[800], color: 'white',
                      borderRadius: '12px', fontWeight: '600', border: 'none', cursor: 'pointer',
                    }}
                    whileHover={motionSafe ? { y: -4, backgroundColor: colors.stone[700] } : {}}
                    whileTap={{ scale: 0.98 }}
                  >
                    Start Building
                  </motion.button>
                ...
                <Link to="/services" className="button-link">
                  <motion.button
                    ...
                    style={{
                      width: '100%', padding: '16px 36px',
                      backgroundColor: 'transparent', color: colors.stone[800],
                      borderRadius: '12px', fontWeight: '600',
                      border: `2px solid ${colors.stone[200]}`, cursor: 'pointer',
                    }}
                    whileHover={motionSafe ? { borderColor: colors.gold[400] } : {}}
                    whileTap={{ scale: 0.98 }}
                  >
                    View Services
                  </motion.button>
              ...
              <style>{`
                .hero-buttons-container { flex-direction: row; }
                .button-link { flex: 1 1 auto; min-width: 200px; }
                @media (max-width: 640px) {
                  .hero-buttons-container { flex-direction: column; }
                  .button-link { width: 100%; min-width: 100%; }
                  .hero-button { width: 100%; text-align: center; }
                }
              `}</style>
            </motion.section>

            {/* Right side - Animated UI Card (desktop only for heavy animations) */}
            <motion.section
              initial="initial" animate="animate"
              style={{ position: 'relative', height: '500px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              {/* Rotating blur orb — desktop only */}
              {motionSafe ? (
                <motion.div
                  style={{
                ...
                <div style={{
                  ...
                  borderRadius: '50%', background: `linear-gradient(135deg, ${colors.gold[100]} 0%, ${colors.gold[300]} 100%)`,
                  filter: 'blur(40px)', zIndex: 1, opacity: 0.7
                }} />
              )}
              ...
              <motion.div
                ...
                style={{
                  ...
                  backdropFilter: motionSafe ? 'blur(20px)' : 'none',
                  WebkitBackdropFilter: motionSafe ? 'blur(20px)' : 'none',
              ...
              >
                  ...
                  <div style={{ position: 'relative', display: 'inline-block' }}>
                    <svg width="120" height="120">
                      ...
                      {motionSafe ? (
                        ...
                        <circle cx="60" cy="60" r="54" fill="none" stroke={colors.gold[400]} strokeWidth="10" strokeLinecap="round"
                          strokeDasharray="339.29" strokeDashoffset="84.8" />
                      )}
                    </svg>
                ...
                {/* Bar chart — animated on desktop, static bars on mobile */}
                <div style={{ display: 'flex', gap: '8px' }}>
                  {[1, 2, 3, 4].map((i, idx) => {
                    const heights = [20, 38, 28, 40]
                    return motionSafe ? (
                      <motion.div
                        key={i}
                      ...
                      <div key={i} style={{ height: `${heights[idx]}px`, flex: 1, backgroundColor: colors.gold[200], borderRadius: '6px' }} />
                    )
                  })}
                </div>
              </motion.div>
                ...
                <>
                  <motion.div
                    style={{
                      ...
                      display: 'flex', alignItems: 'center', gap: '10px', zIndex: 11,
